...
int error_line(int error_index, int total_lines, int* newlines);
...
int main() {
   ...
   if (stream_ptr != NULL) {
      ...
      //will store the total lines of code
      int total_lines = 1;
      size_t error_start = 0;
      //will store the positons of the newline characters
      int newline_positons[MAX_SOURCE_FILE_SIZE];
      size_t k = 0;
      while ((c = source[k]) != '\0') {
         if(c == '\n'){
            newline_positons[total_lines-1] = k;
            total_lines++;
         }
         k++;
      }
      ...
      while ((c = source[source_position]) != '\0') {
         ...
         if (c == '"') {
            ...
            while ((c = source[source_position]) != '"') {
               ...
               if (peep == '\0') {
                  ...
                  //errors
                  strcat(errors, temp);
                  int line = error_line(error_start, total_lines, newline_positons);
         ...
         //identifier
            ...
            do {
               ...
               if ((peep >= 48 && peep <= 57) || (peep >= 65 && peep <= 90) || (peep == 95) || (peep >= 97 && peep <= 122)) {
                  ...
                  if ((peep == 32) || (peep == 33) || (peep == 38) || (peep >= 40 && peep <= 45) || (peep == 47) || (peep >= 59 && peep <= 62) || (peep == 91) || (peep == 93) || (peep == 123) || (peep == 124)) {
                     ...
                     if ((a = strcmp(temp, "function")) == 0) {
                        strcat(symb, "T_FUNCTION\t function\n");
                        memset(temp, 0, strlen(temp));
                        break;
                     }else
            ...
            case ',':
               ...
               break;
            default:
               //that character is not allowed, store those chars that are not part of the language as errors
               ;
               //the semicolon above is important

               size_t j = 0;
               while (!((c >= 32 && c <= 34) || (c == 38) || (c >= 40 && c <= 45) || (c >= 47 && c <= 57) || (c >= 59 && c <= 62) || (c >= 65 && c <= 91) || (c == 93) || (c == 95) || (c >= 97 && c <= 125) || (c == '\0'))) {
                  //save the erroneous identifier until you find a valid delimiter or reach the end of file
                  error_start = source_position;
      ...
      ///a better way is to use a class or a data structure
      ///writing the symbol table into a file to await lexing
      FILE *fhandle;
      fhandle = fopen("symbol_table.txt", "w");
      fprintf(fhandle, "%s", symb);
      fclose(fhandle);
   } else {
...
int error_line(int error_index, int total_lines, int* newlines){
   int line = 0;
   if (error_index == 0){return 1;}
   while(1){
      if(error_index <= *(newlines+line)){
         break;
      }
      line++;
   }
